...
if this does offend you +
that's good — it means you have something you care about in your life

=== agenda

<<the_rise_and_fall_of_managed_runtime,a sip of history>> +
<<malloc_baby,malloc, baby!>> +
<<hello_c,hello, C!>>

== the rise and fall of managed runtime

**\... or why we need a Java Virtual Machine for**
...
the runtime guarantees your program can only touch memory it owns +
no buffer overflows, no dangling pointers, no use-after-free +
*the runtime tracks every allocation and rejects every illegal access*

=== operation safety

the runtime guarantees that every operation is legal for its operands +
you cannot divide a string, cast an object to an incompatible type, or call a method that does not exist +
*the bytecode verifier rejects invalid programs before they ever run*

=== execution safety

the runtime guarantees that control flow can only reach valid code +
you cannot jump into the middle of an instruction, corrupt the call stack, or return to an arbitrary address +
*every branch target and stack frame is verified before execution*

=== !

when you give up control, +
you lose a lot of opportunities to optimize your code +
(notice I didn't say you lose performance)

== Java as a systems language?

stupid idea! +
(especially when resource management is critical)
...
=== so we broke the window

image::https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExeWlwZDc4cDAwcW1hMGdhbmdzOHhpMDNtb3d1dnFucjY2NHVocTBtbiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/2erqKj7wV12bdoPagM/giphy.gif[background]
...
memory addresses are now modeled by zero-length memory segments; they used to be modeled by the now-removed `MemoryAddress` class
...
use `Arena.ofConfined()` to bind a segment to the creating thread — +
any access from other threads throws `WrongThreadException`,

use `Arena.ofShared()` to allow safe concurrent access from multiple threads, +
use `Arena.ofAuto()` for GC-managed lifetime (no explicit close required)

=== dereferencing memory

* to obtain a memory access var handle, clients call `MemoryLayout.varHandle(PathElement... elements)`
* the carrier type (the Java type used to read/write the value) is derived automatically from the selected `ValueLayout` — no explicit `Class<?>` carrier parameter
* alignment constraint and byte order are taken from the selected value layout (e.g. `ValueLayout.JAVA_INT`, `ValueLayout.JAVA_LONG`)
* The resulting var handle has a leading `MemorySegment` coordinate and one `long` coordinate (the base offset), plus one `long` per open path element (e.g. sequence index).
...
* var handles support `VarHandle.AccessMode` operations (e.g. `getAcquire`, `setRelease`, `compareAndSet`) — the `MemoryAccess` utility class was removed in the final API
...
Upcall stubs are modelled by instances of type MemorySegment; upcall stubs can be passed by reference to other downcall method handles and they are released when the associated arena is closed
...
from C header files, to be able to work with any advanced C library

=== a real gem

`jextract` is a standalone tool (no longer bundled with Panama EA builds), +
it generates Java bindings from C header files using the clang C API

https://jdk.java.net/jextract/[jextract Early-Access Builds]

=== jextract

it generates Java code from C header files

    jextract --output ${build.directory}/generated-sources --target-package pl.symentis.foreign.posix /usr/include/mqueue.h

=== what's in it for you?

=== !

image::https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExandiMmVvbHdrMWd4MDR4Y3JsNnQwdW1zdm10eWxoMmp2a2VlYzAyYyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/Z5xk7fGO5FjjTElnpT/giphy.gif[background,contain]

=== !

* when working with huge in-memory data sets, this way you can avoid GC
* interacting with non-JVM and JVM processes over shared memory 
